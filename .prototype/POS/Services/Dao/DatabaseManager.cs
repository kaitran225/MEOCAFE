...
namespace POS.Services.Dao
{
    public class DatabaseManager
    {
        ...
        public DatabaseManager()
        {
            ...
            port = dbConfig.Values["DB_PORT"]?.ToString() ?? Config.GetSetting("DB_PORT") ?? "5432";
            username = dbConfig.Values["DB_USER"]?.ToString() ?? Config.GetSetting("DB_USER") ?? "postgres";
            password = dbConfig.Values["DB_PASSWORD"]?.ToString() ?? Config.GetSetting("DB_PASSWORD") ?? "123456";
            database = dbConfig.Values["DB_NAME"]?.ToString() ?? Config.GetSetting("DB_NAME") ?? "dev_meo_cf";
